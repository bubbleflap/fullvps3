// ════════════════════════════════════════════════════════════════
// DASHBOARD > Recent Bonding section  (card list at /dashboard#recentbonding)
// API: GET /api/bonded-tokens
// DATA: Recently bonded/listed tokens from Flap.sh cache
// SOURCE: cachedListedTokens from Flap.sh GraphQL listed
// DO NOT CONFUSE WITH: /dashboard#bonding (graduated 16 BNB tokens)
// ════════════════════════════════════════════════════════════════
import { useState, useEffect } from "react";
import { RefreshCw, Loader2, X } from "lucide-react";
import TokenCard from "../TokenCard";
import { dashFetchRecentBonding } from "../api";
import { useGlobalSearch } from "../useGlobalSearch";
import { usePagination } from "../usePagination";
import Pagination from "../Pagination";
import type { DToken } from "../types";

export default function DashRecentBonding() {
  ...
  async function load() {
    setLoading(true);
    const data = await dashFetchRecentBonding();
    setTokens(data);
    setLoading(false);
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {paged.map((t, i) => (
              <TokenCard key={t.address || t.ca || i} token={t} rank={(page - 1) * 15 + i + 1} badge="Graduated" badgeColor="bg-green-700" />
            ))}
          </div>
