// ════════════════════════════════════════════════════════════════
// DASHBOARD > Dex Paid section  (card list at /dashboard#dexpaid)
// API: GET /api/dexpaid-tokens → dexPaidService.js
// DATA: Tokens with active DexScreener boosts/paid profiles
// SOURCE: DexScreener boost API only
// DO NOT CONFUSE WITH: /dexpaid (standalone page)
// ════════════════════════════════════════════════════════════════
import { useState, useEffect } from "react";
import { RefreshCw, Loader2, X } from "lucide-react";
import TokenCard from "../TokenCard";
import { dashFetchDexPaid } from "../api";
import { useGlobalSearch } from "../useGlobalSearch";
import { usePagination } from "../usePagination";
import Pagination from "../Pagination";
import type { DToken } from "../types";

export default function DashDexPaid() {
  ...
  async function load() {
    setLoading(true);
    const data = await dashFetchDexPaid();
    setTokens(data);
    setLoading(false);
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {paged.map((t, i) => (
              <TokenCard key={t.address || t.ca || i} token={t} rank={(page - 1) * 15 + i + 1} badge="Dex Paid" badgeColor="bg-yellow-700" />
            ))}
          </div>
