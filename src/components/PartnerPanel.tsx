...
export default function PartnerPanel({ tokens, bondingLoaded, onAskBot, isOpen, onToggle }: PartnerPanelProps) {
  ...
  const handleImgLeave = useCallback(() => setPreview(null), []);

  return (
    <>
      {preview && (
        <div className="fixed z-[500] pointer-events-none" style={{ left: preview.x, top: preview.y }}>
      ...
      >
        <div
          ...
          onClick={() => {
            onToggle();
            if (isOpen) setWidth(DEFAULT_WIDTH);
          }}
        >
          <div className="flex items-center gap-2">
            <h3 className="text-[10px] font-bold text-[#d5f704] uppercase tracking-wider">PARTNER</h3>
            <button className="hover:bg-white/5 p-1 rounded-md transition-colors" title={isOpen ? "Collapse" : "Expand"}>
        ...
        {isOpen && (
          <div className="space-y-1.5 mt-2">
            {loading ? (
              ...
              tokens.map((token) => {
                const isFeatured = (token.ca || "").toLowerCase() === FEATURED_CA;
                return (
                  <button
                    key={token.id}
                    onClick={() => {
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                        {isFeatured ? (
                          ...
                          <span className="font-bold px-1 py-0.5 rounded leading-none flex-shrink-0" style={{ fontSize: `${badgeFontSize}px`, backgroundColor: "#0d6e63", color: "#a7f3d0" }}>
                            PARTNER
                          </span>
                        )}
                        ...
                        {token.taxRate > 0 && (
                          <span className="font-bold px-1 py-0.5 rounded leading-none flex-shrink-0" style={{ fontSize: `${badgeFontSize}px`, backgroundColor: "#92400e", color: "#fde68a" }}>
                            {token.taxRate % 1 === 0 ? token.taxRate.toFixed(0) : token.taxRate.toFixed(1)}% tax
                          </span>
                        )}
                      </div>
              ...
              <div className="text-[9px] text-white/30 text-center py-2">No partner tokens</div>
            )}
          </div>
        )}
        ...
        {isOpen && (
          <div
            ...
            className="absolute bottom-1.5 left-1.5 w-4 h-4 cursor-ew-resize flex items-end justify-start opacity-30 hover:opacity-80 transition-opacity select-none"
