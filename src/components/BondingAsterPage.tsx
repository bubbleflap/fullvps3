...
import { useBondingAsterTokens } from "../hooks/useBondingAsterTokens";
...
export default function BondingAsterPage({ chatBotRef, onPageChange }: Props) {
  const { t } = useLang();
  const { tokens, newestIds, connected, refresh } = useBondingAsterTokens();
    ...
    <>
      <div className="absolute top-[54px] left-1/2 -translate-x-1/2 z-40 pointer-events-none">
        <div className="flex items-center gap-1.5 bg-black/50 backdrop-blur-md rounded-full px-3 py-1 border border-[#7c5af3]/30">
          ...
          <span className="text-[10px] text-white/40">pair &middot; bonding</span>
        </div>
      </div>
      ...
      {tokens.length > 0 && (
        <div className="absolute bottom-[72px] sm:bottom-4 left-1/2 -translate-x-1/2 sm:translate-x-0 sm:left-24 z-40 bg-black/50 backdrop-blur-md rounded-lg px-2.5 py-1 sm:px-3 sm:py-1.5 text-[9px] sm:text-[10px] text-white/30 border border-white/5 whitespace-nowrap">
          {tokens.length} {t.bondingTokens} &middot; ASTER pair
        </div>
      )}
      ...
      {tokens.length === 0 && connected && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="text-center">
            ...
            />
            <div className="text-white/30 text-sm font-medium">No ASTER pair bonding tokens yet</div>
            <div className="text-white/20 text-xs mt-1">Graduating ASTER tokens will appear here</div>
          </div>
        </div>
      )}
    </>
