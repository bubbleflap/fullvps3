...
import { fetchBondingAsterTokens, mapRawToken, NEWEST_POSITIONS } from "../lib/api";

export function useBondingAsterTokens() {
  ...
  const connectWs = useCallback(() => {
    ...
    try {
      ...
      ws.onopen = () => {
        console.log("[AsterBondingPage] WS connected");
        setConnected(true);
        ws.send(JSON.stringify({ type: "subscribe", channel: "bonding-aster" }));
      };
      ...
      ws.onmessage = (event) => {
        try {
          ...
          console.error("[AsterBondingPage] WS parse error:", e);
        }
      };

      ws.onclose = () => {
        console.log("[AsterBondingPage] WS disconnected");
        setConnected(false);
  ...
  useEffect(() => {
    fetchBondingAsterTokens().then((fetched) => {
      if (fetched.length > 0) {
  ...
  const refresh = useCallback(async () => {
    const fetched = await fetchBondingAsterTokens();
    if (fetched.length > 0) {
      const sorted = [...fetched].sort((a, b) => b.createdAt - a.createdAt);
      const positioned = sorted.map((t, i) => {
        if (i < NEWEST_POSITIONS.length) {
