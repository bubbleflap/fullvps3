...
import { fetchNewAsterTokens, mapRawToken, NEWEST_POSITIONS } from "../lib/api";

export function useNewAsterTokens() {
  ...
  const connectWs = useCallback(() => {
    ...
    try {
      ...
      ws.onopen = () => {
        console.log("[AsterNewPage] WS connected");
        setConnected(true);
        ws.send(JSON.stringify({ type: "subscribe", channel: "new-aster" }));
      };
      ...
      ws.onmessage = (event) => {
        try {
          ...
          console.error("[AsterNewPage] WS parse error:", e);
        }
      };

      ws.onclose = () => {
        console.log("[AsterNewPage] WS disconnected");
        setConnected(false);
  ...
  useEffect(() => {
    fetchNewAsterTokens().then((fetched) => {
      if (fetched.length > 0) {
  ...
  const refresh = useCallback(async () => {
    ...
    const fetched = await fetchNewAsterTokens();
    if (fetched.length > 0) {
      const sorted = [...fetched].sort((a, b) => b.createdAt - a.createdAt);
      const positioned = sorted.map((t, i) => {
        if (i < NEWEST_POSITIONS.length) {
