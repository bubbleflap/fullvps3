...
import { fetchNewTokens, mapRawToken, NEWEST_POSITIONS } from "../lib/api";
...
export function useNewTokens() {
  ...
  const connectWs = useCallback(() => {
    ...
    try {
      ...
      ws.onopen = () => {
        console.log("[NewPage] WS connected");
        setConnected(true);
      ...
      ws.onmessage = (event) => {
        try {
          ...
          console.error("[NewPage] WS parse error:", e);
        }
      };

      ws.onclose = () => {
        console.log("[NewPage] WS disconnected");
        setConnected(false);
  ...
  const refresh = useCallback(async () => {
    ...
    const fetched = await fetchNewTokens();
    if (fetched.length > 0) {
      const sorted = [...fetched].sort((a, b) => b.createdAt - a.createdAt);
      const positioned = sorted.map((t, i) => {
        if (i < NEWEST_POSITIONS.length) {
