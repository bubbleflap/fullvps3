...
import { init as dexPaidInit, getDexPaidTokens, getDexPaidMap, markDexPaid, startUpdates as dexPaidStartUpdates, setDetectedMap as dexPaidSetDetectedMap, setDbPool as dexPaidSetDbPool } from './dexPaidService.js';
...
let cachedListedTokens = [];           // graduated → PancakeSwap V2, sorted newest-first
const listedTimestamps = new Map();    // address (lowercase) → ms when first seen in listed
const MAX_LISTED = 75;
const APPROX_LISTING_INTERVAL = 45_000; // ~45s between graduations on Flap.sh
...
initDb().then(async () => {
  ...
  dexPaidSetDbPool(pool);
  dexPaidSetDetectedMap(dexPaidDetectedAtMap);
  dexPaidStartUpdates();
...
async function fetchFlapTokens() {
  ...
  try {
    ...
    try {
      ...
      for (const token of mapped) {
        ...
        if (dex) {
          ...
          token.pairCreatedAt = dex.pairCreatedAt || null;
          // For listed/graduated tokens: use DexScreener price as the real current market cap
          if (dex.priceUsd && token.listed) {
            token.mcap = Math.round(dex.priceUsd * 1_000_000_000);
          }
        }
      }
    } catch (err) {
    ...
    // Listed/graduated section — tokens that passed 16 BNB and are on PancakeSwap
    // Use DexScreener pairCreatedAt as the real graduation timestamp (most accurate source)
    {
      const nowTs = Date.now();
      const seen = new Set();
      cachedListedTokens = mapped
        .filter((t) => {
          if (!t.listed || (t.mcap || 0) < 100 || !t.dexUrl) return false;
          const addr = (t.address || '').toLowerCase();
          if (seen.has(addr)) return false; // deduplicate
          seen.add(addr);
          return true;
        })
        .map((t) => {
          // pairCreatedAt from DexScreener = actual time the PancakeSwap pair was created = graduation time
          const pairTs = t.pairCreatedAt ? new Date(t.pairCreatedAt).getTime() : 0;
          return { ...t, listedAt: pairTs || nowTs };
        })
        .sort((a, b) => (b.listedAt || 0) - (a.listedAt || 0))
        .slice(0, MAX_LISTED);
      const newCount = cachedListedTokens.length;
      if (newCount > 0) console.log(`[LISTED] ${newCount} graduated tokens | newest: ${cachedListedTokens[0]?.ticker} (${Math.round((nowTs - (cachedListedTokens[0]?.listedAt||nowTs))/60000)}m ago) | oldest: ${cachedListedTokens[newCount-1]?.ticker} (${Math.round((nowTs - (cachedListedTokens[newCount-1]?.listedAt||nowTs))/86400000)}d ago)`);
      setHistoricalTokens(cachedListedTokens);
    }
...
app.get("/api/bonding-tokens", async (req, res) => {
  try {
    ...
    console.error('[BONDING-TOKENS]', err.message);
    res.json({ tokens: [] });
  }
});

app.get("/api/bonded-tokens", (req, res) => {
  res.json({ tokens: cachedListedTokens });
});
...
app.get("/api/recent-bonding", async (req, res) => {
  try {
    const tokens = await getBondingListedTokens();
    res.json({ tokens });
  } catch (err) {
    console.error('[RECENT-BONDING]', err.message);
    res.json({ tokens: [] });
  }
});
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => {
    try {
      ...
      const validChannels = ["new", "bonding", "listed", "new-aster", "bonding-aster"];
      if (msg.type === "subscribe" && validChannels.includes(msg.channel)) {
        ...
        } else if (msg.channel === "listed") {
          fetchFlapTokens().then(() => {
            if (ws.readyState !== 1) return;
            if (cachedListedTokens.length > 0) {
              ws.send(JSON.stringify({ type: "bonded_update", tokens: cachedListedTokens }));
            }
          });
        } else if (msg.channel === "bonding") {
          getBondingListedTokens().then((tokens) => {
            if (ws.readyState !== 1) return;
            if (tokens.length > 0) {
              ws.send(JSON.stringify({ type: "tokens_update", tokens }));
            }
          }).catch(() => {});
        } else {
          fetchFlapTokens().then(() => {
            if (ws.readyState !== 1) return;
            if (cachedNewTokens.length > 0) {
              ws.send(JSON.stringify({ type: "tokens_update", tokens: cachedNewTokens }));
            }
          });
        }
      }
...
setInterval(async () => {
  ...
  const listedMsg = cachedListedTokens.length > 0  ? JSON.stringify({ type: "bonded_update",  tokens: cachedListedTokens })  : null;

  let bondMsg = null;
  try {
    const graduatedTokens = await getBondingListedTokens();
    if (graduatedTokens.length > 0) {
      bondMsg = JSON.stringify({ type: "tokens_update", tokens: graduatedTokens });
    }
  } catch {}
  ...
  wss.clients.forEach((client) => {
    ...
    if (client._channel === "listed"  && listedMsg) client.send(listedMsg);
  });
}, 15000);
